...
#include "medianfilter.h"     //20213030 irfilter
float calcDistance(short value) {
  ...
  if(val < 220)      return 100 + 100.0 / (b - a) * (val - a);
  else if(val < 260) return 200 + 50.0  / (bc- b) * (val - b);
  else if(val < 310) return 250 + 50.0 /  (c -bc) * (val - bc);
  else               return 300 + 100.0 / (d - c) * (val - c);
}
...
// Distance sensor
#define _DIST_ALPHA 0.2 //[20203118] DIST_ALPHA 값 설정

// Servo range
#define _DUTY_MIN 1150        //22.5cm (+4cm)  [20213083]서보 각도의 최솟값 설정 
#define _DUTY_NEU 1550        //18.5cm         [20213090]서보 수평 각도 펄스 값
#define _DUTY_MAX 1950        //14.5cm (-4cm)  [20213081]서보 각도의 최댓값

// Servo speed control
#define _SERVO_ANGLE 30 //[20203118] 최대 가동범위에 따른 목표 서보 회전각
#define _SERVO_SPEED 250 //[20203118] 서보 속도 설정
// Event periods
...
// PID parameters
#define _KP 1.1       //비례제어 값
#define _KD 30.0       //미분제어 값
#define _KI 0.01       //적분제어 값
...
//[20203118] 마지막으로 측정한 거리, 마지막으로 측정한 서보 각도(각 이벤트별로 업데이트를 위해 측정하는 시간)
...
dist_raw = dist_ema = filter.read(); // 초기화
